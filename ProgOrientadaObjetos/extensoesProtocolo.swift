...
print("\(penguin.name) é um pássaro que voa? \(penguin.canFly)")
...
struct Dove: Bird, Flyable {
  let name: String
  let amplitube: Double
  let frequency: Double

  var maximumSpeed: Double {
    3 * amplitube * frequency
  }
}
struct Ostrich: Bird {
  let name: String
}
let dove = Dove(name: "Pombo", amplitube: 14.0, frequency: 3.0)
  ...
  var maximumSpeed: Double {
    switch self {
      ...
      case .european: 
        return 9.9
      case .unknown: 
        fatalError("Não sabemos a velocidade.")
    }
  }
}
extension Swallow {
...
print("Velocidade máxima dos pilotos \(topSpped(of: racers))")
...
protocol Cheat {
  mutating func boots(_ power: Double)
}
